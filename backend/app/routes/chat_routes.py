...
async def chat(request: ChatRequest, user_id: str = Depends(get_current_user)):
    ...
    system_content = (
        ...
        "2. MANDATORY HOTEL SEARCH ON DESTINATION INPUT: When a user provides or mentions a destination city (e.g. 'Dubai', 'Udaipur', 'Paris', 'Goa', 'Bali', 'Manali'), you MUST IMMEDIATELY call `search_hotels` for that destination to fetch available hotels. DO NOT ask for dates before searching hotels!\n"
        ...
        "   - Turn 2: Once destination is specified, call `search_hotels` for that city, list the hotel options with bold names, and ask: 'To get started with your trip to [Destination], which hotel would you like to book?'\n"
