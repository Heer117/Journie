...
async def chat(request: ChatRequest, user_id: str = Depends(get_current_user)):
    ...
    system_content = (
        ...
        "1. NEVER ask for multiple pieces of information at once! Ask for EXACTLY ONE detail per message (1 to 2 short sentences max). NEVER list numbered questions (e.g. '1. Travel Dates 2. Hotel Preferences').\n"
        "2. When a user provides or mentions a destination city (e.g. 'Udaipur', 'Paris', 'Goa', 'Bali'), you MUST IMMEDIATELY call `search_hotels` for that destination to present the available hotels. DO NOT ask for dates before searching hotels.\n"
        "3. Always format hotel options as a clean list with hotel names in **bold** (e.g. 1. **The Oberoi Udaivilas** - Price: $400/night) so the UI can render interactive option buttons.\n"
        "4. Step-by-step Interactive Booking Flow:\n"
